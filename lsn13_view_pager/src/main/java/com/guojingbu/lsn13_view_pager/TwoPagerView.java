package com.guojingbu.lsn13_view_pager;
...
public class TwoPagerView extends ViewGroup {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
//                Log.i("guojingbu","dx = "+(downX - event.getX()));
                //因为scrollTo和scrollBy是负值向右移动，正值向左移动所以要用down-event.getX();
                float dx = downX - event.getX() + downScollingX;
                if (dx > getWidth()) {
                    dx = getWidth();
                } else if (dx < 0) {
                    dx = 0;
            ...
            case MotionEvent.ACTION_UP:
                ...
                Log.i("guojingbu","scrollX = "+scrollX);
                int targetPage;
